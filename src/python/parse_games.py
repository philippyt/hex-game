...
def convert_board_to_graph(board: np.ndarray, graph_id: int, graphs_obj: Graphs, board_dim: int) -> None:
    ...
    for i in range(board_dim):
        for j in range(board_dim):
            ...
            # count valid neighbors for this specific node
            valid_neighbors = 0
        ...
        for j in range(board_dim):
            ...
            for off in neighbors:
                ...
                if 0 <= ni < board_dim and 0 <= nj < board_dim:
                    ...
                    dst_val = board[ni][nj]
                    edge_type = (
                        f"SameColor_{'Pos' if src_val == 1 else 'Neg'}"
                        if src_val == dst_val and src_val != 0
                        else "Adjacent"
                    )
                    graphs_obj.add_graph_node_edge(graph_id, src, dst, edge_type)
...
def create_graphs_from_games(
    ...
    for graph_id, _ in enumerate(games):
